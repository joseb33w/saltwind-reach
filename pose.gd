...
##   GPose.ride(character)  -> the ASTRIDE preset (Wave 3 mounts): legs spread ~40° OUTWARD
##                             (about the character's forward axis, per-side), knees bent ~70°
##                             back, torso upright (spine untouched), hands gently forward to
##                             the reins. Same true/false contract and the SAME "gpose_state"
##                             meta snapshot as sit — one restore path for both.
...
# --- ride (astride) preset ---
# RIDE_SPREAD_DEG: thighs swung OUTWARD about the character's FORWARD (+Z) axis, each to its own
# side (sign from the thigh's lateral offset). RIDE_KNEE_DEG: knees bent BACK about the
# (spread-carried) lateral axis — POSITIVE where sit's THIGH_DEG -85 is forward.
const RIDE_SPREAD_DEG := 40.0
const RIDE_KNEE_DEG := 70.0
...
## Pose `character` ASTRIDE a mount (Wave 3): legs spread ~40° outward + knees bent ~70° back,
## torso upright (spine untouched), hands gently toward the reins. Returns false (silent no-op)
...
static func ride(character: Node3D) -> bool:
	...
	var orig := {}                       # bone idx -> original local pose Quaternion (for stand())
	# ---- per-thigh OUTWARD spread about the forward axis. Rotating a down-pointing (-Y) leg
	# about +Z_char by +θ swings its tip toward +X_char ((0,-1,0) -> (sinθ, -cosθ, 0)), so a
	# thigh offset to the +X side spreads outward with a POSITIVE angle and the -X side with a
	# NEGATIVE one. Side = the thigh origin's offset along the lateral axis; a rig that centres
	# both thighs (degenerate) falls back to alternating signs so the legs still split.
	var spread := {}                     # thigh idx -> that side's spread Basis
	var flip := 1.0
	for t in thighs:
		...
		if absf(side) < 0.001:
			sgn = flip
			flip = -flip
		spread[t] = Basis(fwd, deg_to_rad(RIDE_SPREAD_DEG) * sgn)
	# thighs: spread outward (global left-multiply, converted to local — the sit() conversion)
	for t in thighs:
		var tp := skel.get_bone_parent(t)
		var tpg: Basis = (g[tp] as Basis) if tp >= 0 else Basis.IDENTITY
		_set_local(skel, t, tpg.inverse() * ((spread[t] as Basis) * (g[t] as Basis)), orig)
	# shins: bend the knee ~70° BACK about the knee's POST-SPREAD lateral axis (the thigh's
	# lateral axis carried through the spread), on top of the inherited thigh delta — feet tuck
	# along the mount's flanks.
	for s in shins:
		var sp := skel.get_bone_parent(s)
		var owner_t := _ancestor_of(skel, s, thighs)
		if sp < 0 or owner_t < 0:
			continue                     # shin not under a matched thigh: leave it alone
		var srot: Basis = spread[owner_t] as Basis
		var knee := Basis((srot * axis).normalized(), deg_to_rad(RIDE_KNEE_DEG))
		var spg: Basis = srot * (g[sp] as Basis)   # parent chain inherited the spread delta
