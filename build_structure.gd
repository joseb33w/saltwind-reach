...
static func structure(spec: Dictionary, proxy: bool = false) -> Node3D:
	...
	# optional CLOCK faces (a clocktower landmark): an emissive dial + hands on each wall, high up
	if bool(spec.get("clock", false)):
		_add_clock_faces(root, foot, height)
	...
	# whole-AABB box here would seal the doorway shut.
	if not shelled:
...
# Four emissive clock dials + hands near the top of a tower (the "clock": true spec key).
static func _add_clock_faces(root: Node3D, foot: Vector2, height: float) -> void:
	var y := height * 0.84
	var dial_r := minf(foot.x, foot.y) * 0.32
	var face_mat := StandardMaterial3D.new()
	face_mat.albedo_color = Color(0.96, 0.93, 0.82)
	face_mat.emission_enabled = true
	face_mat.emission = Color(1.0, 0.92, 0.6)
	face_mat.emission_energy_multiplier = 1.6
	var hand_mat := StandardMaterial3D.new()
	hand_mat.albedo_color = Color(0.12, 0.1, 0.08)
	var setups := [
		[Vector3(0, y, foot.y * 0.5 + 0.10), 0.0],
		[Vector3(0, y, -foot.y * 0.5 - 0.10), PI],
		[Vector3(foot.x * 0.5 + 0.10, y, 0), PI * 0.5],
		[Vector3(-foot.x * 0.5 - 0.10, y, 0), -PI * 0.5],
	]
	for s in setups:
		var pivot := Node3D.new()
		pivot.position = s[0]
		pivot.rotation.y = s[1]
		root.add_child(pivot)
		var dial := MeshInstance3D.new()
		var cm := CylinderMesh.new()
		cm.top_radius = dial_r
		cm.bottom_radius = dial_r
		cm.height = 0.12
		dial.mesh = cm
		dial.rotation.x = PI * 0.5   # cylinder axis Y -> face +Z
		dial.material_override = face_mat
		pivot.add_child(dial)
		var hour := MeshInstance3D.new()
		var hb := BoxMesh.new()
		hb.size = Vector3(0.10, dial_r * 0.55, 0.05)
		hour.mesh = hb
		hour.position = Vector3(0, dial_r * 0.18, 0.11)
		hour.rotation.z = deg_to_rad(35.0)
		hour.material_override = hand_mat
		pivot.add_child(hour)
		var minute := MeshInstance3D.new()
		var mb := BoxMesh.new()
		mb.size = Vector3(0.08, dial_r * 0.8, 0.05)
		minute.mesh = mb
		minute.position = Vector3(0, dial_r * 0.25, 0.12)
		minute.rotation.z = deg_to_rad(-70.0)
		minute.material_override = hand_mat
		pivot.add_child(minute)
...
static func _interior_spec(spec: Dictionary) -> Dictionary:
	...
	if typeof(iv) == TYPE_DICTIONARY:
		...
		if door != "arch":
			door = "hinged"
		var face := String(d.get("door_face", "s")).to_lower()
