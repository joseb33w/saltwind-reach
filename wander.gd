...
## crowd, not statues). Cheap: pure transform movement (no physics body), but COLLISION-AWARE: a short raycast
## validates each stroll target and probes ahead while walking, so agents route around solid objects instead of
## clipping through them (colliderless worlds: rays hit nothing -> behaviour identical to before). Movement ticks
...
const RAY_H := 1.0          # obstacle rays fly ~1m above the ground (torso height — over kerbs, under archways)
const PROBE_AHEAD := 1.5    # while walking, look this far ahead for something solid
const PROBE_EVERY := 0.6    # seconds between forward probes (cheap: ~2 rays/s per moving agent)
...
# TRUE when a solid collider sits between XZ points `a` and `b` (the ray flies RAY_H above the terrain at each
# end). Space-state access — only valid during _physics_process. Colliderless worlds: intersect_ray returns an
# EMPTY Dictionary (never an error), so this is always FALSE and behaviour matches the pre-collision build.
func _blocked(a: Vector2, b: Vector2) -> bool:
	var w := get_world_3d()
	if w == null:
		return false
	var from := Vector3(a.x, _ground(a.x, a.y) + RAY_H, a.y)
	var dest := Vector3(b.x, _ground(b.x, b.y) + RAY_H, b.y)
	var q := PhysicsRayQueryParameters3D.create(from, dest)   # default mask, bodies only
	q.exclude = _exclude
	return not w.direct_space_state.intersect_ray(q).is_empty()
