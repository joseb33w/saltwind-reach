...
func serialize() -> Dictionary:
	return {
		"hp": hp, "max_hp": max_hp, "level": level, "xp": xp, "xp_next": xp_next,
		"gold": gold, "inventory": inventory.duplicate(), "equipped": equipped_weapon,
...
func restore(d: Dictionary) -> void:
	...
	xp = int(d.get("xp", xp))
	xp_next = int(d.get("xp_next", xp_next))
	gold = int(d.get("gold", gold))
	var inv = d.get("inventory", null)
	if inv is Array:
		inventory = (inv as Array).duplicate()
	var eq := String(d.get("equipped", equipped_weapon))
	if eq != "" and eq in inventory:
		equipped_weapon = eq
	var fl = d.get("flags", null)
	if fl is Dictionary:
		flags = (fl as Dictionary).duplicate()
	changed.emit()
...
func item_name(id: String) -> String:
	...
	return ITEMS.get(id, {}).get("name", id.capitalize())
