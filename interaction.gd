...
# A generic USE target with a caller-supplied callback (the tavern's RELIC HUNTERS plaque,
# and any future info point). The visual is the caller's; this only registers the trigger.
func add_custom(pos: Vector3, label: String, cb: Callable, cell_key := "") -> void:
	items.append({kind = "custom", pos = pos, label = label, cb = cb, cell = cell_key})
...
func _nearest(rng: float):
	...
	for it in items:
		...
		if it.kind == "vehicle":
			...
			it.label = (vn as Vehicle).prompt_label()     # "Drive …" parked / "Exit …" while driven
		if it.kind == "seat":
			var sn = it.get("node")
...
func _open_door(it: Dictionary) -> void:
	...
	if it.lock != "" and not rpg.has_item(it.lock) and not rpg.has_flag(it.lock):
		var lk := String(it.lock)
		if lk.ends_with("_key") or rpg.ITEMS.has(lk):
			_show(["The vault door is sealed tight.", "You need the " + rpg.item_name(lk) + "."])
		else:
			_show(["The door will not budge yet."])
		return
	it.open = true
...
func _talk(it: Dictionary) -> void:
	...
	if main_ref != null and main_ref.has_method("on_npc_talked") and String(it.get("npc_id", "")) != "":
		main_ref.on_npc_talked(String(it.npc_id))   # Explore mode: the keeper can start the Adventure
