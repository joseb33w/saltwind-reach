...
const FAR_RADIUS := 3                  # far-PROXY ring radius: silhouette-only cells past the resident ring
const FAR_CAP := 48                    # LRU ceiling on live far proxies (bounds far-ring memory)
...
func build_proxy(rec: Dictionary, gx: int, gz: int) -> Node3D:
	...
	if terrain != null:
		...
		root.add_child(mi)
	# ROADS: the same terrain-following builder residents use (its collider container is removed
	# by the final _strip_physics pass below)
...
func build_cell(rec: Dictionary, gx: int, gz: int) -> Dictionary:
	...
	if interaction != null:
		...
		# RELIC HUNTERS plaque (or any info board): a readable bronze panel whose USE callback
		# asks main for the live leaderboard panel. Data-driven: {"plaque": {pos, facing, title}}.
		var plq = rec.get("plaque", null)
		if typeof(plq) == TYPE_DICTIONARY and world_main.has_method("show_leaderboard"):
			var qp := _xz(plq.get("pos", [0, 0]))
			var qpos := centre + Vector3(clampf(qp.x, -half + 1.0, half - 1.0), 0.0, clampf(qp.y, -half + 1.0, half - 1.0))
			qpos.y = _ground_y(qpos.x, qpos.z)
			_build_plaque(root, qpos, float(plq.get("facing", 0.0)), String(plq.get("title", "RELIC HUNTERS")))
			interaction.add_custom(qpos, "Read the plaque", Callable(world_main, "show_leaderboard"), ckey)
...
# The tavern's wall plaque: a dark timber board with a bronze face + engraved title, on a post.
func _build_plaque(root: Node, pos: Vector3, facing: float, title: String) -> void:
	...
	var pm := BoxMesh.new()
	pm.size = Vector3(0.12, 1.5, 0.12)
	post.mesh = pm
	post.position.y = 0.75
	post.material_override = GSurf.surface("timber")
	pivot.add_child(post)
	var board := MeshInstance3D.new()
	var bm := BoxMesh.new()
	bm.size = Vector3(1.2, 0.9, 0.08)
	board.mesh = bm
	board.position.y = 1.7
	board.material_override = GSurf.surface({"color": [0.45, 0.32, 0.14], "rough": 0.35, "metal": 0.8})
	pivot.add_child(board)
	var lbl := Label3D.new()
	lbl.text = title
	lbl.font_size = 48
	lbl.pixel_size = 0.006
	lbl.modulate = Color(1.0, 0.9, 0.55)
	lbl.outline_size = 8
	lbl.position = Vector3(0.0, 1.85, 0.05)
	pivot.add_child(lbl)
	var sub := Label3D.new()
	sub.text = "fastest to the Sunstone"
	sub.font_size = 24
	sub.pixel_size = 0.006
	sub.modulate = Color(0.95, 0.85, 0.6)
	sub.position = Vector3(0.0, 1.62, 0.05)
	pivot.add_child(sub)
...
func _road_strip(root: Node, centre: Vector3, dir: String, width: float) -> void:
	...
	var body := StaticBody3D.new()   # parallel shape container (one body, seg_n shapes)
	body.collision_layer = 1
...
func _place_structure(root: Node, spec: Dictionary, centre: Vector3, half: float, cell_key := "") -> void:
	...
	var p := centre + Vector3(clampf(xz.x, -half + 0.5, half - 0.5), 0.0, clampf(xz.y, -half + 0.5, half - 0.5))
	# grounding sample: normally the structure's own pos; "ground_at" overrides it (cell-local
	# [x,z]) so a span (a bridge deck over the carved river) anchors to its BANK height instead
	# of the channel bottom under its centre.
	var ga = spec.get("ground_at", null)
	if ga is Array and (ga as Array).size() >= 2:
		var gxz := _xz(ga)
		p.y = _ground_y(centre.x + gxz.x, centre.z + gxz.y)
	else:
		p.y = _ground_y(p.x, p.z)   # sit the building base on the terrain surface
	# optional vertical offset (e.g. a bridge deck sunk flush with its banks so the top is walkable)
	p.y += float(spec.get("y_off", 0.0))
	node.position = p
	root.add_child(node)
...
func _place_one(root: Node, ref, centre: Vector3, half: float, cell_key := "") -> bool:
	...
	# optional vertical offset ABOVE the ground sample — furniture on an interior storey slab
	# ({"y": <floor_height>} puts a bed on the upstairs floor of an enterable structure)
	n.position.y += float(ref.get("y", 0.0))
...
# Grounded by the source mesh's base offset. SOLID-BY-DEFAULT (shared SOLID_MIN_DIM contract): an
# entry big enough to block also gets a PARALLEL per-instance shape container (the visual stays
# one draw call); tiny clutter (grass/pebbles) stays walkthrough.
func _place_scatter(root: Node, ref, centre: Vector3, half: float) -> void:
	...
	if mesh != null:
		...
		if _collider_mode(cspec, AABB(Vector3.ZERO, msz * base)) != "none":
	...
	for i in range(mini(cnt, 6)):
		...
		# same solid-by-default rule for the duplicated-node fallback (derived box from world AABB)
		if _collider_mode(cspec, ab) != "none":
